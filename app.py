...
def weather():
    url = "https://www.bbc.co.uk/weather/0/359783"
    data = requests.get(url).text
    soup = BeautifulSoup(data, 'html.parser')
    today = soup.find('div', {'data-component-id': 'forecast'})
    temp = today.find(class_='wr-value--temperature--c').get_text()
    return temp
...
def seven_days_weather():
    ...
    data = requests.get(url).text
    soup = BeautifulSoup(data, 'html.parser')
    seven_days = soup.find_all(class_='wr-day-temperature__high-value')
    seven_dates = soup.find_all(class_='wr-date__longish')
    id= 1
    id2= 1
    for days in seven_days:  
        day = days.find(class_='wr-value--temperature--c').get_text()
        cursor.execute("UPDATE seven_days_weather SET weather = ? WHERE id=?", (day, id))
        db.commit()
        id = id+1
        print(day)
    for dates in seven_dates:
        date = dates.get_text()
        print(date)
        cursor.execute("UPDATE seven_days_weather SET date  = ? WHERE id=?", (date, id2))
        db.commit()
        id2 = id2+1        

def fetch_seven_days():
    cursor.execute(
        "SELECT date, weather FROM seven_days_weather")
    seven_days_weather = cursor.fetchall()
    return seven_days_weather


def local_time():
    url = "https://www.timeanddate.com/worldclock/egypt"
    data = requests.get(url).text
    soup = BeautifulSoup(data, 'html.parser')
    today = soup.find(id="tz-legend-3").get_text()
    return today


def currency():
    url = "https://www.poundsterlinglive.com/best-exchange-rates/convert/best-pound-to-egyptian-pound-exchange-rate"
    data = requests.get(url).text
    soup = BeautifulSoup(data, 'html.parser')
    rate = soup.find(id="mini1").get_text()
    # test = rate.find("span").get_text()
    return rate
...
def index():
    seven_days_weather()
    seven_days = fetch_seven_days()
    today_weather = weather()
    current_time = local_time()
    currency_exchange = currency()
    guest_comments = fetch_guest_comment()
    return render_template('index.html', page_title=page_title[0], guest_comments=guest_comments, LOGGED_IN=LOGGED_IN, LOGGED_OUT=LOGGED_OUT, today_weather=today_weather, local_time=current_time, currency_exchange=currency_exchange, seven_days=seven_days)
